...
TYPED_TEST(HashMap, emplace_or_get_returns_an_iterator_and_true_when_successful)
{
...
TYPED_TEST(HashMap, emplace_or_get_for_an_existing_element_returns_an_iterator_to_that_element_and_false)
{
...
TYPED_TEST(HashMap, get_or_insert_calls_factory_and_returns_iteratur_to_newly_inserted_element)
{
...
TYPED_TEST(HashMap, get_or_insert_does_not_call_factory_and_returns_iterator_to_existing_element)
{
  bool called_factory = false;
...
TYPED_TEST(HashMap, containts_returns_false_for_non_existing_element)
{
  EXPECT_FALSE(this->map.contains(43));
}

TYPED_TEST(HashMap, find_returns_end_iterator_for_non_existing_element)
{
  EXPECT_EQ(this->map.end(), this->map.find(43));
}

TYPED_TEST(HashMap, contains_returns_true_for_existing_element)
{
  this->map.emplace(42, 43);
  EXPECT_TRUE(this->map.contains(42));
}

TYPED_TEST(HashMap, find_returns_iterator_to_existing_element)
{
  this->map.emplace(42, 43);
...
TYPED_TEST(HashMap, erase_nonexisting_element_returns_false)
{
  EXPECT_FALSE(this->map.erase(42));
}

TYPED_TEST(HashMap, erase_existing_element_returns_true_and_removes_element)
{
  this->map.emplace(42, 43);
  EXPECT_TRUE(this->map.erase(42));
}

TYPED_TEST(HashMap, erase_existing_element_twice_fails_the_seond_time)
{
  this->map.emplace(42, 43);
  EXPECT_TRUE(this->map.erase(42));
  EXPECT_FALSE(this->map.erase(42));
...
TYPED_TEST(HashMap, begin_returns_iterator_to_first_entry)
{
  this->map.emplace(42, 43);
...
TYPED_TEST(HashMap, drain_densely_populated_map_using_erase)
{
  for (int i = 0; i < 200; ++i)
...
TYPED_TEST(HashMap, drain_sparsely_populated_map_using_erase)
{
  for (int i = 0; i < 4; ++i)
...
TYPED_TEST(HashMap, iterator_covers_all_entries_in_densely_populated_map)
{
...
TYPED_TEST(HashMap, iterator_covers_all_entries_in_sparsely_populated_map)
{
  std::map<int, bool> values;
  for (int i = 0; i < 4; ++i) {
    values[i * 7] = false;
