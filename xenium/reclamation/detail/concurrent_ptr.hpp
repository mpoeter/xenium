...
namespace xenium { namespace reclamation { namespace detail {
  ...
  public:
    ...
    marked_ptr load(std::memory_order order = std::memory_order_seq_cst) const { return ptr.load(order); }

    // Atomic store.
    void store(const marked_ptr& src, std::memory_order order = std::memory_order_seq_cst) { ptr.store(src, order); }
