...
namespace xenium { namespace reclamation {
  ...
  private:
    void ensure_has_control_block() {
      if (control_block == nullptr) {
        ...
        do {
          ...
          !global_epoch.compare_exchange_weak(epoch, epoch, std::memory_order_acq_rel, std::memory_order_relaxed));
      }
    }
    ...
    bool try_update_epoch(unsigned curr_epoch, unsigned new_epoch) {
      ...
      bool cannot_update =
        std::any_of(global_thread_block_list.begin(), global_thread_block_list.end(), prevents_update);
      if (cannot_update)
        return false;
  ...
  inline quiescent_state_based::region_guard::region_guard() noexcept { local_thread_data().enter_region(); }

  inline quiescent_state_based::region_guard::~region_guard() noexcept { local_thread_data().leave_region(); }
  ...
  inline void quiescent_state_based::count_allocation() { local_thread_data().allocation_counter.count_allocation(); }

  inline void quiescent_state_based::count_reclamation() { local_thread_data().allocation_counter.count_reclamation(); }
#endif
